...
switch _mode do {
	...
	case "Mouse": {
		...
		_isActive = {
			{BIS_fnc_camera_keys param [_x, false]} count (actionkeys _this) > 0
		};

		_coef = 0.1;
		if ("cameraMoveTurbo1" call _isActive) then {_coef = _coef * 10;};
		if ("cameraMoveTurbo2" call _isActive) then {_coef = _coef * 100;};
		if (BIS_fnc_camera_keys select 0x36) then {_coef = _coef / 10;};
	...
	case "KeyDown": {
		...
		switch (true) do {
			...
			case (_key == 0x02): {1 call _camSave;};
			case (_key == 0x03): {2 call _camSave;};
			case (_key == 0x04): {3 call _camSave;};
			case (_key == 0x05): {4 call _camSave;};
			case (_key == 0x06): {5 call _camSave;};
			case (_key == 0x07): {6 call _camSave;};
			case (_key == 0x08): {7 call _camSave;};
			case (_key == 0x09): {8 call _camSave;};
			case (_key == 0x0A): {9 call _camSave;};
			case (_key == 0x0B): {0 call _camSave;};
			...
			case (_key == 0x39): {
				...
				_alt = if (_vehicle != player) then {getposatl _vehicle select 2} else {_worldPos select 2}; 
