...
std::vector<GameValue*> VariableDumper::GetMissionVariables() {
	...
	for (auto i = 0; i < CurrentNamespace.GetSize(); i++) {
		...
		for (auto j = 0; j < NamespaceEntry.GetSize(); j++) {
			auto Value = NamespaceEntry.GetAddr<GameValue*>(j, 0x28);
...
void VariableDumper::RenderMenu() {
	auto Variables = GetMissionVariables();
	...
	if (ImGui::Button("Dump Functions")) {
		for (auto& GameValue : Variables) {
			if (GameValue->GetVariableType() != VariableType::CODE)
				continue;
	...
	if (ImGui::BeginListBox("##VARIABLES", ImVec2(350, ImGui::GetContentRegionAvail().y - 40))) {
		...
		for (int i = 0; i < Variables.size(); i++) {
			...
			if (Variables[i]->GetVariableName().find(InputSearchBuffer) == std::string::npos)
				continue;

			if (ImGui::Selectable(std::to_string(i).c_str(), Selected)) {
				VariableTarget = i;

				if (Variables[VariableTarget]->GetVariableType() == VariableType::CODE) {
			...
			{
			case VariableType::SCALAR:	{ ImGui::TextColored(ImColor(000, 255, 000),	"%s - Number",		Variables[i]->GetVariableName().c_str());	} break;
			case VariableType::ARRAY:	{ ImGui::TextColored(ImColor(000, 255, 255),	"%s - Array",		Variables[i]->GetVariableName().c_str());	} break;
			case VariableType::STRING:	{ ImGui::TextColored(ImColor(128, 255, 128),	"%s - String",		Variables[i]->GetVariableName().c_str());	} break;
			case VariableType::CODE:	{ ImGui::TextColored(ImColor(255, 255, 000),	"%s - Code",		Variables[i]->GetVariableName().c_str());	} break;
			case VariableType::BOOL:	{ ImGui::TextColored(ImColor(255, 255, 255),	"%s - Bool",		Variables[i]->GetVariableName().c_str());	} break;
			case VariableType::GROUP:	{ ImGui::TextColored(ImColor(255, 000, 255),	"%s - Group",		Variables[i]->GetVariableName().c_str());	} break;
			case VariableType::OBJECT:	{ ImGui::TextColored(ImColor(255, 000, 255),	"%s - Object",		Variables[i]->GetVariableName().c_str());	} break;
			default:					{ ImGui::TextColored(ImColor(255, 000, 000),	"%s - Unkn",		Variables[i]->GetVariableName().c_str());	} break;
			}
		...
		switch (Variables[VariableTarget]->GetVariableType()) {
		...
		case VariableType::CODE: {
			...
			if (RestorePtr.second != 0)
				ImGui::TextColored(ImColor(0, 255, 0), "Restore: %s \n", RestorePtr.first.c_str());
			else
				ImGui::TextColored(ImColor(255, 0, 0), "Restore: NOTCOPIED \n");
			...
			if (ImGui::Button("Place Ptr")) {
				RestorePtr.first = Variable->GetVariableName();
				RestorePtr.second = Variable->GetValuePtr();
			...
			if (ImGui::Button("Restore Ptr") && !Variable->GetVariableName().compare(RestorePtr.first)) {
				Variable->SetValuePtr(RestorePtr.second);
				SQFCodeEditor.SetText(Variable->GetValue());
